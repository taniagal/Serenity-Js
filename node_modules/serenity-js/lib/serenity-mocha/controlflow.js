...
function makeAsyncTestFn(fn) {
    ...
    if (isAsync && isGenerator) {
        throw TypeError('generator-based tests must not take a callback; for async testing,'
    ...
    var ret = (function (done) {
        ...
        var runTest = function (resolve, reject) {
            try {
                if (isAsync) {
                    fn.call(_this, function (err) { return err ? reject(err) : resolve(); });
                }
                else if (isGenerator) {
                    resolve(promise.consume(fn, _this));
                }
                else {
                    resolve(fn.call(_this));
                }
            }
            catch (ex) {
                reject(ex);
