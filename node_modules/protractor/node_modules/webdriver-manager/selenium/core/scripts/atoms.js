  ...
  try {
    ...
    if (!goog.userAgent.IE || goog.userAgent.isDocumentModeOrHigher(9)) {
      for (var f = {}, g = d.getElementsByTagName("*"), h = 0;h < g.length;++h) {
        var k = g[h], m = k.namespaceURI;
        if (!f[m]) {
          var l = k.lookupPrefix(m);
          if (!l) {
            var p = m.match(".*/(\\w+)/?$"), l = p ? p[1] : "xhtml"
          }
          f[m] = l;
        }
      }
...
bot.dom.getClientRect = function(a) {
  function b(a) {
    ...
    if (c) {
      var d = parseFloat(c[1]), e = parseFloat(c[2]), l = parseFloat(c[3]), p = parseFloat(c[4]), n = parseFloat(c[5]), c = parseFloat(c[6]), q = f.left + f.width, r = f.top + f.height, t = f.left * d, d = q * d, u = f.left * e, e = q * e, v = f.top * l, l = r * l, w = f.top * p, q = r * p, r = t + v + n, p = u + w + c, v = d + v + n, w = e + w + c, t = t + l + n, u = u + q + c, n = d + l + n, c = e + q + c;
...
bot.dom.appendVisibleTextLinesFromElement_ = function(a, b) {
  if (bot.dom.isElement(a, goog.dom.TagName.BR)) {
    ...
    var h = bot.dom.isShown(a), k = null, m = null;
...
bot.events.TouchEventFactory_.prototype.create = function(a, b) {
  ...
  var f = goog.dom.getOwnerDocument(a), g = goog.dom.getWindow(f), h = e(b.changedTouches), k = b.touches == b.changedTouches ? h : e(b.touches), m = b.targetTouches == b.changedTouches ? h : e(b.targetTouches), l;
...
bot.action.drag = function(a, b, c, d, e, f) {
  ...
  for (var h = 1;h <= d;h++) {
    var k = Math.floor(h * b / d), m = Math.floor(h * c / d), l = bot.dom.getClientRect(a), k = new goog.math.Coordinate(e.x + g.left + k - l.left, e.y + g.top + m - l.top);
    f.move(a, k);
  }
  f.releaseButton();
};
